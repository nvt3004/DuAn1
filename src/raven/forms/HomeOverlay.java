...
public class HomeOverlay extends JWindow {
    ...
    public class PanelOverlay extends JPanel {
        ...
        private Animator SiginAnimator;
        private float animate;
        private int index;
        private boolean showLogin;
        private boolean showSigin;
        ...
        private void init() {
            ...
            SiginAnimator = new Animator(500, new Animator.TimingTarget() {
                @Override
                public void timingEvent(float v) {
                    float f = showSigin ? v : 1f - v;
                    int x = (int) ((350 + 180) * f);
                    migLayout.setComponentConstraints(panelSigin, "pos 100%-" + x + " 0.5al, w 350");
                    revalidate();
                }
            });
            animator.setInterpolator(CubicBezierEasing.EASE_IN);
            SiginAnimator.setInterpolator(CubicBezierEasing.EASE);
        ...
        private void createSigin() {
            panelSigin = new Sigin();
            add(panelSigin, "pos 100% 0.5al,w 350");
        }
        ...
        private void runSiginAnimation(boolean show) {
            if (showSigin != show) {
                if (!SiginAnimator.isRunning()) {
                    showSigin = show;
                    SiginAnimator.start();
                }
            }
